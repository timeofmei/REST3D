...
if [[ $# -lt 3 ]]; then
    echo "Usage: $0 SCENE_TREE SCENE_DIR NEW_OUTPUT_DIRECTORY [extra replay args]" >&2
    exit 2
fi
...
nvrtc_library_dir="$(dirname "${nvrtc_library}")"
driver_library_path="${nvrtc_library_dir}:${wsl_driver_lib_dir}"
...
"${isaaclab_python}" "${repo_root}/scripts/replay_in_simulator.py" \
    --backend isaac-lab \
